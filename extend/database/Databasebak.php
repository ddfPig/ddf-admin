...
class Databasebak
{
    ...
    public function backup($table, $start){
        //备份表结构
        if(0 == $start){
            $result = \Think\Db::query("SHOW CREATE TABLE `{$table}`");
            $sql  = "\n";
            $sql .= "-- -----------------------------\n";
        ...
        //数据总数
        $result = \Think\Db::query("SELECT COUNT(*) AS count FROM `{$table}`");
        ...
        if($count){
            ...
            //备份数据记录
            $result = \Think\Db::query("SELECT * FROM `{$table}` LIMIT {$start}, 1000");
            foreach ($result as $row) {
                // $row = array_map('mysql_real_escape_string', $row);
                $sql = "INSERT INTO `{$table}` VALUES ('" . implode("', '", str_replace('\\','\\\\',$row)) . "');\n";
                if(false === $this->write($sql)){
                    return false;
                }
            }
